...
import { cancelBookingAction, completeBookingAction } from '@/features/pujas/actions';
...
export default async function PujaBookingsPage({ searchParams }: BookingsPageProps) {
  ...
  const result = await pujaService().listBookings(ctx, { status, page: page ?? 1 });
  if (!result.ok) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Paid bookings from your website. Mark them completed once the puja is performed.
        </p>
      </div>
