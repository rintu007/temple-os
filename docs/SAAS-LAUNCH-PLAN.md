...
- ✅ Payment failure/retry visibility for temple staff. Audit found the actual gap: `payment_orders.status` had a `'failed'` enum value that was **dead code** — no webhook/callback across any of the three gateways ever wrote it, so a failed devotee payment just sat at `'created'` forever, invisible to staff. Fixed: Razorpay's `payment.failed` webhook event and Stripe's `checkout.session.expired` event now call `orderRepo.markFailed()` (which never overwrites an already-`'paid'` order, guarding against out-of-order webhook delivery); the SSLCommerz return-leg does the same best-effort when the gateway's fail/cancel POST includes a `tran_id`. New `/payments/failed` admin page (Finance nav) lists failed orders plus `'created'` orders stale over an hour (abandoned mid-checkout, no webhook ever fired). Verified with live-DB tests (`payment.service.test.ts`) covering both the record path and the never-clobber-a-paid-order guard. **Requires action on your end to fully activate**: register `payment.failed` in the Razorpay dashboard webhook config and `checkout.session.expired` in the Stripe dashboard (doc comments on both webhook routes note this) — without those, this still works for SSLCommerz but Razorpay/Stripe failures still won't be recorded until the new event types are added there.
...
- ✅ Expanded the homepage feature section to cover the whole system, not just 4 highlights — grouped exactly along the real module tiers (Free forever / Worship / Community & fundraising / Full fund accounting), each with real capability bullets pulled from the actual admin nav (puja/seva/darshan/prasadam booking, membership, volunteers, WhatsApp broadcasts, payroll, loans, investments, grants, 80G tax receipts, etc.) so it flows straight into the pricing grid below it. Also added a 4-step "How it works" section and a homepage-level FAQ (data security, country support, no-code claim, "what happens if I stop paying"). Verified in a real browser (Playwright, production build, localhost root — not a tenant subdomain) with zero console errors before shipping.
...
- ✅ Formal pre-launch security review (flagged as a risk in the original `docs/DEVELOPMENT-PLAN.md` §13; conducted as an internal code audit, not a third-party pen test). Checked: RLS coverage across all 78 tables against every policy file (no gaps), all SECURITY DEFINER functions (`search_path` pinned, no dynamic SQL, EXECUTE restricted), Stripe/Razorpay/SSLCommerz webhook signature verification, the admin middleware bypass list (checked specifically for another M79a-style broad-prefix leak — none found), permission gating on financial actions (expenses/payroll/loans), injection/SSRF surface, secrets handling, IDOR on tenant-scoped reads, and devotee magic-link token generation. **No critical/high/medium findings.** Two low-severity notes: no CSP/HSTS/X-Frame-Options headers set (both now addressed, see below), and the devotee magic-link token travels in a URL query param (mitigated: single-use, 15-min expiry, 192-bit random — standard pattern, flagged for completeness only). The one real process gap it surfaced — nothing currently enforces that a new Drizzle table gets a matching RLS policy — is addressed below.
- ✅ `pnpm db:verify-rls` (`packages/db/scripts/verify-rls.mjs`) now asserts every `public` table has RLS **enabled and forced**, not just prints statuses for a human to eyeball — fails with a per-table PASS/FAIL list otherwise. There's no GitHub Actions CI in this repo to wire it into automatically (would need DB secrets added to GitHub settings, which only you can do) — so for now this is a required manual step after any schema/RLS change, not an automated gate. Running it immediately caught a **real, pre-existing gap**: `permissions` had `ENABLE ROW LEVEL SECURITY` but was missing `FORCE ROW LEVEL SECURITY` (present on every other table) — fixed in `packages/db/sql/0001_rls_policies.sql`. Low real-world exposure in practice (the app role isn't the table owner, so RLS applied regardless of FORCE), but a genuine inconsistency worth closing. Re-verified clean against the live DB: all 78 tables pass.
- ✅ Basic security headers (HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy) added to both apps via `next.config.ts`.
- ✅ Content-Security-Policy, added and live on both apps — the item above was originally deferred because a wrong CSP silently breaks checkout with nothing but a browser-console warning, and donation intake must never break. Rather than ship a policy built from static source-reading alone, it was verified against a **real production build** driven by a headless browser (Playwright) through a live Razorpay test-mode checkout (order creation → modal → submit), plus the gallery page (Supabase Storage images) and the homepage — zero CSP violations in the end. That live run caught two Razorpay sub-origins static analysis would have missed entirely: `cdn.razorpay.com` (their risk-detection script, only loaded once the modal opens) and `lumberjack.razorpay.com` (their analytics beacon) — both now allowlisted alongside `checkout.razorpay.com`/`api.razorpay.com`. `apps/admin` gets a stricter policy (no Razorpay allowance needed there). `'unsafe-inline'` on `script-src`/`style-src` is a deliberate, documented compromise (Next.js App Router injects inline hydration scripts without a CSP-nonce pipeline wired through middleware) — noted inline in both `next.config.ts` files rather than left as a silent gap.
...
- ✅ Uptime/error monitoring + alerting (M85) — `/api/cron/health-check` (admin) checks DB connectivity directly and pings the new `apps/sites` `/api/health` route; emails every platform admin only on an up→down or down→up transition, via the existing Resend pattern (no new third-party pinger). **Now registered and running daily at 8am UTC** (`apps/admin/vercel.json`, confirmed live via `vercel cron ls`). Note this settles the open question from earlier: **this Vercel account is on the Hobby plan**, which rejected a `*/5 * * * *` schedule outright at deploy time ("Hobby accounts are limited to daily cron jobs") — so this only catches outages that are still ongoing once a day, not brief blips. Upgrading to Pro is the path to real sub-daily monitoring if that's ever wanted.
...
- 🟡 Status page (M85 follow-up) — `/status` on the sites app reads live from `health_checks` and shows per-service up/down + last-checked time, linked from the marketing footer. Honest when data is missing ("Not yet monitored") rather than faking green. The health-check cron is now live (daily, see above), so this will populate after its first run. Still currently only reachable via the real custom domain, same interim-demo-mode limitation as `/pricing` and `/terms` (confirmed: both 404 on the bare `templeos-sites.vercel.app` URL too, not a regression).
- ✅ Onboarding email sequence — the day-1/3/7 drip is now built (M83 follow-up): `platform_subscriptions` gained three `onboarding_dayN_sent_at` columns, a SECURITY DEFINER `app_list_orgs_for_onboarding_nudges()` (same cross-tenant-scan pattern as trial reminders) finds orgs due for their next nudge, and `/api/cron/onboarding-nudges` sends exactly one nudge per org per run — the earliest un-sent day-N threshold crossed, so a missed run catches up next time without ever double-sending. Verified with a live-DB test (`billing.service.test.ts`) exercising the full list→mark-sent→re-list cycle. **Now registered and running daily at 10am UTC**, confirmed live via `vercel cron ls`. Content: day 1 nudges devotees + website, day 3 nudges team invites, day 7 highlights worship bookings + donations as trial-only value.
...
- [x] **M84** — Admin-side changelog/announcement mechanism. New platform-wide `changelog_entries` table (not tenant-scoped) with `changelog_reads` tracking each user's read cursor, same "derive unread from a per-user cursor" shape as the existing notification tray but without an `organizationId` (entries aren't per-org events). RLS: any signed-in user can read (`app_current_user_id() IS NOT NULL`), only platform admins can write (`app_is_platform_admin()`) — same public-read/admin-write split as the plan catalog. Platform admins publish entries at `/platform/changelog`; every user sees a "What's new" bell (`Megaphone` icon) in the dashboard topbar next to the existing notification bell. Core-layer authorization mirrors `plan.service.ts` (`repo.isPlatformAdmin(actorUserId)` on every write, `FORBIDDEN` otherwise) — defense in depth alongside the RLS backstop. Verified with 6 tests against the live shared DB (create/read/unread-per-user/mark-read/delete, denied for non-admins), exercising the actual production RLS policies end to end; admin build succeeded and the new `/platform/changelog` route correctly redirects unauthenticated requests in production.
- [x] **M85** — Uptime/error monitoring + alerting. New platform-wide `health_checks` table (last known status per service, same open-RLS shape as `rate_limits`) plus `app_list_platform_admin_emails()` (SECURITY DEFINER, mirrors `app_list_trials_ending_soon`) so the cron — which has no browser session — knows who to alert. `checkAndRecord`-style logic only fires an email on an actual state transition, never on every check, and never on a service's first-ever observation (no false "recovered" email on day one). Deployed to both apps and verified live (`GET /api/health` on sites → `{"status":"up"}`; the admin cron route correctly 401s without the bearer secret) — but the Cron schedule itself is deliberately not registered yet, so nothing calls it on its own. **Still needs**: deciding a schedule and adding it back to `apps/admin/vercel.json`.
- [ ] **M86+** — Security review, backup-restore drill, structured audit-log retention, support channel, status page — sequenced once §5.6 is answered.
