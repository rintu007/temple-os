...
- [ ] **M85+** — Security review, backup-restore drill, uptime/error monitoring + alerting, support channel, status page — sequenced once §5.6 is answered. Note on monitoring specifically: sub-daily Vercel Cron schedules may be capped to once/day depending on the Vercel plan tier, and real uptime alerting usually wants a third-party pinger (which would need your account) — worth deciding the approach before building.
