...
- 🟡 Uptime/error monitoring + alerting (M85) — `/api/cron/health-check` (admin) checks DB connectivity directly and pings the new `apps/sites` `/api/health` route; emails every platform admin only on an up→down or down→up transition, via the existing Resend pattern (no new third-party pinger). Deployed and confirmed working when invoked directly, but **the Vercel Cron schedule is intentionally left unregistered** (removed from `apps/admin/vercel.json` at your request) — nothing calls it automatically yet. Wire it back in (`*/5 * * * *` or similar) whenever you're ready; that will also be the first real signal on whether this Vercel plan tier allows sub-daily cron frequency.
