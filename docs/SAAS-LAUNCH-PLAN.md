...
- ✅ Basic security headers (HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy) added to both apps via `next.config.ts`. **CSP deliberately not added**: `apps/sites` dynamically loads `checkout.razorpay.com/v1/checkout.js` and opens a Razorpay payment iframe for donations — a wrong CSP silently breaks checkout with nothing but a browser-console warning, and this codebase treats donation intake as something that must never break. Needs verifying against a live Razorpay/Stripe/SSLCommerz checkout run before enforcing; noted inline in both `next.config.ts` files.
