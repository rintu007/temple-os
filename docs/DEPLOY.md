...
| `CRON_SECRET` | any random string (e.g. `openssl rand -hex 32`) | admin only. Authenticates Vercel Cron's daily call to `/api/cron/trial-reminders` (`apps/admin/vercel.json`) — Vercel sends `Authorization: Bearer $CRON_SECRET` automatically once this is set. Also requires `STRIPE_BILLING_WEBHOOK_SECRET` to be configured for the *other* billing email (payment-failed) to fire, since that one is triggered from the Stripe webhook, not the cron. |
